...
def collide(obj1, obj2):
    ...
    return obj1.mask.overlap(obj2.mask, (offset_x, offset_y)) != None #return True if collide
...
def main():
    ...
    player_vel = 5
    laser_vel = 6
    main_font = pygame.font.Font((os.path.join("assets", "pcsenior.ttf")), 20)
    ...
    while run:
        ...
        if keys[pygame.K_a] and player.x - player_vel > 0: #to left
            player.x -= player_vel
        if keys[pygame.K_d] and player.x + player_vel + player.get_width() < WIDTH: #to right
            player.x += player_vel
        if keys[pygame.K_w] and player.y - player_vel > 0: #to up
            player.y -= player_vel
        if keys[pygame.K_s] and player.y + player_vel + player.get_height() < HEIGHT: #to down
            player.y += player_vel   
...
def menu():
    title_font = pygame.font.Font((os.path.join("assets", "pcsenior.ttf")), 20)
    run = True
    while run:
        WIN.blit(BG, (0, 0))
